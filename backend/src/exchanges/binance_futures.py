"""
Binance Futures Connector - ccxt ile Binance USDT-M Futures API.

Config'ten okur: exchange.name, exchange.api_key, exchange.api_secret, exchange.testnet
"""
...
class BinanceFuturesExchange(BaseExchange):
    ...
    def get_balance(self) -> float:
        """USDT cinsinden kullanılabilir bakiye (futures wallet)."""
    ...
    def get_positions(self, symbol: Optional[str] = None) -> List[Dict[str, Any]]:
        """Açık pozisyonlar. symbol None ise tümü. ccxt: contracts + veya - ile yön verir."""
    ...
    def get_klines(
        ...
        """OHLCV. ccxt: [timestamp, open, high, low, close, volume]."""
    ...
    def place_order(
        ...
        """
        Market veya limit order. reduce_only için params kullanılır.
        """
        params = {}
        if reduce_only:
        ...
        if order_type == "market":
            ...
            # Limit için fiyat gerekir; bu interface'de opsiyonel bırakıyoruz
            # İleride stop_price limit fiyatı olarak kullanılabilir
            order = self._client.create_order(
                symbol=symbol,
                type="limit",
                side=side,
